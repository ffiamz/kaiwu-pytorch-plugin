...
1. **编码器（Encoder）**  
   将输入数据 $\mathbf{x}$ 映射为潜变量的近似后验分布  
   $q_\phi(\mathbf{z}|\mathbf{x})$，通常由神经网络参数化。
...
- **负相（negative phase）**：从 QBM 中采样 $\mathbf{z} \sim p_\theta(\mathbf{z})$，使用 **蒙特卡洛方法** 或 **量子退火器**
